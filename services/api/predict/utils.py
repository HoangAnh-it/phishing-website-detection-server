...
def preprocess_input(url, html):
    ...
    with open(os.path(settings.MODEL_STORAGE, "url_tokenizer.pickle"), "rb") as handle:
        url_tokenizer = pickle.load(handle)

    with open(os.path(settings.MODEL_STORAGE, "html_tokenizer.pickle"), "rb") as handle:
        html_tokenizer = pickle.load(handle)

    url_sequences = url_tokenizer.texts_to_sequences(np.array(url))
    html_sequences = html_tokenizer.texts_to_sequences(np.array(html))
